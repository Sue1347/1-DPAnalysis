...
time_points = np.linspace(1, max(df["PFS"])-1, max(df["PFS"])-1)  # Create time points within the range
print(time_points)
...
for var in var_list:
    ...
    # Step 3: Calculate sensitivity, specificity, and thresholds
    marker = df[var].values  # Continuous variable
    def calculate_time_dependent_roc(df, marker, time_point):
        # Determine which patients are at risk at the given time_point
        at_risk = df['PFS'] >= time_point
        true_event = (df['PFS'] <= time_point) & (df['event'])

        # Get marker values for at-risk patients
        marker_values = marker[at_risk]
        true_labels = true_event[at_risk]
        if true_labels.any() == False: return [0],[0],[0],0,0

        # Compute ROC curve
        fpr, tpr, thresholds = roc_curve(true_labels, marker_values)
        ...
        return fpr, tpr, thresholds, optimal_threshold, youden_index[optimal_idx]

    tpoint_ = optimal_threshold_ = youden_index_ = 0
    for tpoint in range(max(df["PFS"])-2):
        fpr, tpr, thresholds, optimal_threshold, youden_index = calculate_time_dependent_roc(
            df, marker, time_points[tpoint]
        )
        if youden_index > youden_index_: 
            youden_index_ = youden_index
            optimal_threshold_ = optimal_threshold
            tpoint_ = tpoint
    print(f"Optimal threshold for marker at time {time_points[tpoint_]}: {optimal_threshold_}")
    print(f"Youden index : {youden_index_}")
        # # Step 4: Plot the ROC curve
        # plt.plot(fpr, tpr, label=f"ROC Curve (AUC={auc_scores[tpoint]:.2f})")
...
Kaplan_Meier_two_plot(df_ori, 'Age_thrshd') # 'Age_thrshd', 'SUVmaxBM_thrshd','SUVmaxFL_thrshd','SUVmaxEMD_thrshd',